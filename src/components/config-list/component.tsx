...
const ConfigList: React.FC<ConfigListProps> = ({
  ...
  const [activeValue, setActiveValue] = useState<number>(initialActiveDuration ?? 30);
  ...
  const totalTimeFormatted = () => {
    const totalInSeconds = STARTING_ROUND_DURATION + roundsValue * activeValue + roundsValue * waitValue - waitValue;
        ...
        <IonItem className="config-item">
          <div className="config-item-wrapper">
            <div className="config-item-label">
              ...
              {item.description && <IonLabel className="config-item-text">{item.description}</IonLabel>}
            </div>
            <NumberSetter initValue={item.initialValue} updateValue={item.valueUpdater} />
          </div>
        </IonItem>
      </>
    );
          ...
          <div className="list-item-end-position-wrapper">
            <IonItem>
              <div className="list-item-end-position-item">
                <IonLabel>
                  {'Total time '}
                  <span style={{ border: '1px solid white', borderRadius: '0.5rem', padding: '0.2rem' }}>
                    {totalTimeFormatted()}
                  </span>
                </IonLabel>
              </div>
